...
public class MututalTests extends Assert {
	...
	public static void testParseDefaultValueNotExistingAttribute(TestParserFactory p) {
		TestBean bean = p.create().parse("<testBean/>");
		assertNotNull(bean);
		assertEquals("defaultone", bean.attrone);
		assertEquals("defaulttwo", bean.getAttr2());
		assertEquals("defaultthree", bean.getAttrthree());
		assertEquals(2, bean.attrint);
		assertEquals(new Integer(3), bean.attrinteger);
		assertEquals(4, bean.attrlong);
		assertEquals(new Long(5L), bean.attrLong);
		assertEquals(6, bean.attrshort);
		assertEquals(new Short("7"), bean.attrShort);
		assertEquals(8f, bean.attrfloat);
		assertEquals(9f, bean.attrFloat);
		assertEquals(10.0, bean.attrdouble);
		assertEquals(11.0, bean.attrDouble);
		assertTrue(bean.attrboolean);
		assertTrue(bean.attrBoolean);
	}
	...
	public static void testParseSubObjectList(TestParserFactory p) {
		TestBean bean = p.create().parse(
				"<testBean><objectList attrone=\"passed1\"/><objectList attrone=\"passed2\"/><objectList attrone=\"passed3\"/></testBean>");
		assertNotNull(bean);
	...
	public static void testParseSubObjectPolymorphicList(TestParserFactory p) {
		TestBean bean = p
				.create()
				.parse("<testBean><objectList attrone=\"passed1\"/><objectList xsi:type=\"TestSubObjectChild\" attrtwo=\"passed3\" attrone=\"passed2\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"/></testBean>");
		assertNotNull(bean);
		assertNotNull(bean.objectList);
	...
	public static void testParseNamedSubObjectPolymorphicList(TestParserFactory p) {
		TestBean bean = p.create().parse(
				"<testBean><namedList xsi:type=\"namedObject\" objattr=\"passed3\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"/></testBean>");
		assertNotNull(bean);
	...
	public static void testPolymprohicElement(TestParserFactory p) {
		TestBean bean = p.create().parse(
				"<testBean><abstractObj xsi:type=\"testImplObject\" val=\"true\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"/></testBean>");
		assertNotNull(bean);
	...
	public static void testListStringElement(TestParserFactory p) {
		TestBean bean = p
				.create()
				.parse("<testBean><abstractObj xsi:type=\"testImplObject\" val=\"true\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"/><stringsList>test</stringsList><stringsList>test2</stringsList><stringsList>test3</stringsList></testBean>");
		assertNotNull(bean);
